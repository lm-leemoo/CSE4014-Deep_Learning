...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        temp = dw
        temp += 2*self.reg*self.params['W2']
        grads['b2'] = db
        grads['W2'] = temp
        dx, dw, db  = Linear_ReLU.backward(dx, cache_LR)
        temp2 = dw
        temp2 += 2*self.reg*self.params['W1']
        grads['b1'] = db
        grads['W1'] = temp2
        ###################################################################
        #                     END OF YOUR CODE                            #
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, len(hidden_dims)):
          self.params[f'W{i+1}'] = torch.zeros(last_dim, hidden_dims[i], dtype=dtype, device=device)
          self.params[f'b{i+1}'] = torch.zeros(hidden_dims[i], dtype=dtype, device=device)
          self.params[f'W{i+1}'] += weight_scale * torch.randn(last_dim, hidden_dims[i], dtype=dtype, device=device)
          last_dim = hidden_dims[i]
        self.params[f'W{len(hidden_dims) + 1}'] = torch.zeros(last_dim, num_classes, dtype=dtype, device=device)
        self.params[f'b{len(hidden_dims) + 1}'] = torch.zeros(num_classes, dtype=dtype, device=device)
        self.params[f'W{len(hidden_dims) + 1}'] += weight_scale * torch.randn(last_dim, num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for n  in range(self.num_layers-1):
          i=n+1
          last_out, temp_cache = Linear_ReLU.forward(last_out,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
          cache_dict['cache_LR{}'.format(i)] = temp_cache
          if self.use_dropout:
            last_out, temp_cache =  Dropout.forward(last_out,self.dropout_param)
            cache_dict['cache_Dropout{}'.format(i)] = temp_cache
        i+=1
        last_out, temp_cache = Linear.forward(last_out,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
        cache_dict['cache_L{}'.format(i)] = temp_cache
        scores = last_out
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        # Replace "pass" statement with your code
        loss, dout = softmax_loss(scores, y)
        temp = (self.params['W{}'.format(i)]*self.params['W{}'.format(i)]).sum()
        temp *= self.reg
        loss += temp
        last_dout, dw, db  = Linear.backward(dout, cache_dict['cache_L{}'.format(i)])
        grads['b{}'.format(i)] = db
        temp2 = dw
        temp2 += 2*self.params['W{}'.format(i)]*self.reg
        grads['W{}'.format(i)] = temp2
        for n  in range(self.num_layers-1)[::-1]:
          ...
          grads['b{}'.format(i)] = db
          temp3 = dw
          temp3 += 2*self.params['W{}'.format(i)]*self.reg
          grads['W{}'.format(i)] = temp3
...
def rmsprop(w, dw, config=None):
    ...
    config['cache'] += dw_sq * (1 - config['decay_rate'])
    learning_rate_adjustment = config['learning_rate'] * dw / (torch.sqrt(config['cache']) + config['epsilon'])
    w -= learning_rate_adjustment
    next_w = w
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    m_update = (1 - config['beta1']) * dw
    m_update += config['beta1'] * config['m']
    config['t'] += 1
    v_update = (dw * dw) * (1 - config['beta2'])
    v_update += config['beta2'] * config['v']
    config['m'] = m_update
