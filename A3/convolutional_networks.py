          ...
          for f in range(F):
            for height in range(H_dout):
              for width in range(W_dout):
                temp = x[n,:,height*stride:height*stride+HH,width*stride:width*stride+WW] * dout[n,f,height,width]
          ...
          for c in range(C):
            for wi in range(W1):
              for hi in range(H1):
                x_start = wi * stride 
                y_start = hi * stride
                pool_region = x[n, c, y_start:y_start + pool_height, x_start:x_start + pool_width]
                max_val = torch.max(pool_region)
                out[n, c, hi, wi] = max_val
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C):
            for wi in range(W1):
              for hi in range(H1):
                ...
                y_start = hi * stride
                pool_region = x[n, c, y_start:y_start + pool_height, x_start:x_start + pool_width]
                max_idx = torch.argmax(pool_region)
                i_y = max_idx // pool_width
                i_x = max_idx % pool_width
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        W_out_conv = (W + 2 * conv_param['pad'] - WW) // conv_param['stride'] + 1
        H_out_conv = (H + 2 * conv_param['pad'] - HH) // conv_param['stride'] + 1
        W_out = (W_out_conv - pool_param['pool_width']) // pool_param['stride'] + 1
        H_out = (H_out_conv - pool_param['pool_height']) // pool_param['stride'] + 1
        ...
        flattened_size = num_filters*H_out*W_out
        self.params['W2'] = torch.zeros(flattened_size, hidden_dim, dtype=dtype, device=device)
        ...
        self.params['W2'] += weight_scale * torch.randn(flattened_size, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        W_squared_sum = 0
        for i in range(1,4):
          W_squared_sum += (self.params['W{}'.format(i)] * self.params['W{}'.format(i)]).sum()
        loss += W_squared_sum * self.reg
        last_dout, dw, db  = Linear.backward(dout, cache_L)
        temp1 = 2*self.params['W{}'.format(1)]*self.reg
        temp2 = 2*self.params['W{}'.format(2)]*self.reg
        temp3 = 2*self.params['W{}'.format(3)]*self.reg
        grads['b{}'.format(3)] = db 
        grads['W{}'.format(3)] = temp3 + dw
        last_dout, dw, db  = Linear_ReLU.backward(last_dout, cache_LR)
        grads['b{}'.format(2)] = db
        grads['W{}'.format(2)] = temp2 + dw
        last_dout, dw, db  = Conv_ReLU_Pool.backward(last_dout, cache_CRP)
        grads['b{}'.format(1)] = db
        grads['W{}'.format(1)] = temp1 + dw
        ###################################################################
        #                             END OF YOUR CODE                    #
        ###################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i,num_filter in enumerate(num_filters):
          ...
          temp_H_out = H_out + 2 * conv_param['pad']
          temp_W_out = W_out + 2 * conv_param['pad']
          W_out = int(1 + (temp_W_out - WW) / conv_param['stride'])
          H_out = int(1 + (temp_H_out - HH) / conv_param['stride'])
          if self.batchnorm:
          ...
          if i in max_pools:
            temp_H_out = H_out - pool_param['pool_height']
            temp_W_out = W_out - pool_param['pool_width']
            W_out = int(1 + temp_W_out / pool_param['stride'])
            H_out = int(1 + temp_H_out / pool_param['stride'])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          if i in self.max_pools:
            if self.batchnorm:
              out,cache[layer_key] = Conv_BatchNorm_ReLU_Pool.forward(out,self.params['W{}'.format(i)], self.params['b{}'.format(i)],
              self.params['gamma{}'.format(i)],self.params['beta{}'.format(i)],conv_param,self.bn_params[i],pool_param) 
            else:  
              out,cache[layer_key] = Conv_ReLU_Pool.forward(out,self.params['W{}'.format(i)], self.params['b{}'.format(i)],conv_param,pool_param) 
          else:
            if self.batchnorm:
              out,cache[layer_key] = Conv_BatchNorm_ReLU.forward(out,self.params['W{}'.format(i)], self.params['b{}'.format(i)],
            self.params['gamma{}'.format(i)],self.params['beta{}'.format(i)],conv_param,self.bn_params[i]) 
            else:
              out,cache[layer_key] = Conv_ReLU.forward(out,self.params['W{}'.format(i)], self.params['b{}'.format(i)],conv_param) 
        i+=1
        layer_key = str(i)
        out, cache[layer_key] = Linear.forward(out,self.params['W{}'.format(i)], self.params['b{}'.format(i)])
        scores = out
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        for i in range(i-1,-1,-1):
          ...
          if i in self.max_pools:
            if self.batchnorm:
              last_dout, dw, db, dgamma1,dbetta1  = Conv_BatchNorm_ReLU_Pool.backward(last_dout, cache[layer_key])
              temp = 2*self.reg*self.params[f'gamma{i}']
              temp2 = 2 *self.reg*self.params[f'W{i}']
              grads[f'beta{i}'] = dbetta1
              grads[f'b{i}'] = db
              grads[f'W{i}'] = temp2 + dw
              grads[f'gamma{i}'] = temp + dgamma1
            else:  
              last_dout, dw, db  = Conv_ReLU_Pool.backward(last_dout, cache[layer_key])
              grads[f'b{i}'] = db
              grads[f'W{i}'] = 2 * self.params[f'W{i}'] * self.reg + dw
          else:
            if self.batchnorm:
              last_dout, dw, db,dgamma1,dbetta1  = Conv_BatchNorm_ReLU.backward(last_dout, cache[layer_key])
              temp3 = 2*self.reg*self.params[f'gamma{i}']
              temp4 = 2 *self.reg*self.params[f'W{i}']
              grads[f'beta{i}'] = dbetta1
              grads[f'b{i}'] = db
              grads[f'gamma{i}'] = temp3 + dgamma1
              grads[f'W{i}'] = temp4 + dw
            else:
              last_dout, dw, db  = Conv_ReLU.backward(last_dout, cache[layer_key])
              grads[f'b{i}'] = db
              grads[f'W{i}'] = 2 * self.params[f'W{i}'] * self.reg + dw
        #############################################################
        #                       END OF YOUR CODE                    #
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        random_noise = torch.randn(Din, Dout, dtype=dtype, device=device)
        weight += random_noise * weight_scale
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        random_noise = torch.randn(Din,Dout, K,K, dtype=dtype,device= device)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            normalized = (x - running_mean) * (1.0 / sqrtvar)
    ...
    def backward(dout, cache):
        ...
        dvar = 0.5 * dsqrtvar * 1. /torch.sqrt(var+eps)
        dsq = dvar * 1. /N * torch.ones((N,D),device = dout.device)
        dxmu1 = ivar * dxhat
        dxmu2 = 2 * dsq * xmu
        dx1 = (dxmu1 + dxmu2)
        dmu = -1 * torch.sum(dxmu1+dxmu2, axis=0)
        dx2 = dmu * 1. /N * torch.ones((N,D),device = dout.device)
        dx = dx1 + dx2
    ...
    def backward_alt(dout, cache):
        ...
        dout_term = N * dout - dbeta - dgamma * xhat
        dx =  dout_term * (gamma * ivar / N)
        ############################### ##################################
        #                        END OF YOUR CODE                       #
        #################################################################
