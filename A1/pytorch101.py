...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_value = start + 10 - (start % 10) if start % 10 != 0 else start
    if start_value > stop:
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    matrix = torch.eye(max(x)+1, dtype=torch.float32)
    y = matrix[x]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    x_reshaped = x_reshaped.view(2, 3, 4)
    y = x_reshaped.permute(1, 0, 2).contiguous().view(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    val, indices = torch.min(x, dim =1)
    y[torch.arange(x.size(0)), indices] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sum = x.sum(dim=0)
    mean = sum / M
    variance = ((x-mean)**2).sum(dim=0) / (M - 1)
    std = variance.sqrt()
    std[std == 0] = 1
    y = (x - mean) / std
    ##########################################################################
