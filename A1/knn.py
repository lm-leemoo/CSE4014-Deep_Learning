...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_flat = x_train.view(x_train.shape[0], -1)
    test_flat = x_test.view(x_test.shape[0], -1)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      dists[i, :] = torch.sum((train_flat[i] - test_flat) ** 2, dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_flat = x_train.view(num_train, -1)
    test_flat = x_test.view(num_test, -1)
    train = torch.sum(train_flat ** 2, dim=1, keepdim=True)
    test = torch.sum(test_flat ** 2, dim=1, keepdim=True)
    # Compute the distances
    dists = train + test.T - 2 * torch.mm(train_flat, test_flat.T)  # (num_train, num_test)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      ...
      y_pred[i] = torch.mode(closest_y).values.item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
